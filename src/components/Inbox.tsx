...
export default function Inbox({ creds, onReply, onReplyAll }: { creds: MailCredentials; onReply: (m: OpenMessage) => void; onReplyAll: (m: OpenMessage) => void }) {
  const [items, setItems] = useState<Summary[]>([]); const [open, setOpen] = useState<OpenMessage | null>(null);
  ...
  return <section className="card inbox-card"><div className="card-head"><h2>Bandeja de entrada</h2><button type="button" className="ghost small" onClick={() => load(0)}>Actualizar</button></div><p className="hint">Exmail IMAP · la contraseña solo se usa durante esta sesión.</p>{status && <p className="inbox-status">{status}</p>}
    ...
    {open && <div className="mail-detail"><div className="card-head"><div><b>{open.subject}</b><p className="hint">{open.from.name || open.from.address} · {open.date && new Date(open.date).toLocaleString()}</p></div><div className="reply-actions"><button className="ghost small" type="button" onClick={() => onReply(open)}>Responder</button><button className="primary small" type="button" onClick={() => onReplyAll(open)}>Responder a todos</button></div></div><iframe title="Correo recibido" sandbox="allow-popups" srcDoc={open.html} /></div>}
  </section>;
}
